...
export default function CharacterUpdateForm(props) {
  const {
    id: idProp,
    character: characterModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = characterRecord
      ? { ...initialValues, ...characterRecord }
      : initialValues;
  ...
  const [characterRecord, setCharacterRecord] =
    React.useState(characterModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Character, idProp)
        : characterModelProp;
      setCharacterRecord(record);
    };
    queryData();
  }, [idProp, characterModelProp]);
  React.useEffect(resetStateValues, [characterRecord]);
  const validations = {
    name: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Character.copyOf(characterRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CharacterUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || characterModelProp)}
